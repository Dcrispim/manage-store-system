...
class CartItem(models.Model):
    ...
    def __str__(self):
        mode = 'VENDA' if self.opid.mode==0 else 'COMPRA'
        return str(f'({mode}:{self.opid.pk}) {self.product.name}')


class Service(models.Model):
    description = models.CharField(max_length=100)
    itens = models.ManyToManyField(Stock, related_name="itemService")
    material = models.ManyToManyField(Stock, related_name="materialService")
    labor = models.DecimalField(max_digits=8, decimal_places=2)
    off   = models.DecimalField(max_digits=3, decimal_places=2)
